...
function draw() {
  ...
  scene = (current_time < 95000 || current_time > 200000) ? 'A' : 'B'
  ...
  if (current_time >= lastMeasureTime + measureDuration) {
    ...
    processMIDINotes(current_time, level, scene);
  }
  ...
  moon.near(dusts, 10 + random(40) + level * 200);
  ...
  let radius = map(micLevel, 0, 0.2, 0, 100); 
  fill('#38bdf822');  
  noStroke();
  //let _above = 100 + random(20);   
  let _above = 50 + random(-40,20);   
  //ellipse(kpos_x, kpos_y - _above, 0.5+radius);
  ellipse(moon.x, moon.y + _above, 0.5+radius);

  // Create or add to the flock when the mic level exceeds a threshold
  if (micLevel > 0.05) {  
    let newBirds = 10 + random(10); // Number of birds to add
    for (let i = 0; i < newBirds; i++) {
      //let b = new Bird(kpos_x, kpos_y - _above);
      let b = new Bird(moon.x, moon.y + _above);
      flock.addBird(b);
    }
  }
...
function processMIDINotes(currentTime, level, scene='A') {
  ...
  if (scene==='B') {
    falling_points.push(new FallingPoint(kpos_x, random(0.1,0.4)*height))
    if(level>0.1)
      falling_points.push(new FallingPoint(kpos_x+random(30), random(0.1,0.4)*height))
  }
